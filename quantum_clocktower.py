...
from chars import teensy_char_list, teensy_token_list, Player, Character, Token, get_character
...
if evils_predetermined == "y":
    evils_predetermined = True
elif evils_predetermined == "n":
    evils_predetermined = False
else:
    raise ValueError
if not 4 < player_count < 16:
    raise ValueError
if player_count < 10:
    evil_count = 2
    ...
    evil_count = 4
if player_count in [5, 7, 10, 13]:
    outsider_count = 0
elif player_count in [6, 8, 11, 14]:
    outsider_count = 1
else:
    outsider_count = 2
...
def create_night_action(seat: Seat):
    ...
    chosen_player_box = ttk.Combobox(night_action_frame, state="readonly", values=[None]+[p.name for p in player_list])
    chosen_player_box.grid(row=1)
    info_type_label = tk.Label(night_action_frame, text="Info Learned")
    info_type_label.grid(row=2)
    info_type = ttk.Combobox(night_action_frame, state="readonly", values=[None, "Number", "One Character", "Three Characters"])
    info_type.grid(row=3, sticky="n")
    info_type.bind("<<ComboboxSelected>>", lambda event: show_specific_info_box(event.widget.get(), night_action_frame))
    specific_info_box = None
    night_action_done_button = tk.Button(night_action_panel, text="Done", command=lambda: finish_night_action(seat.player, chosen_player_box.get(), info_type.get(), night_action_panel))
...
def show_specific_info_box(info_type: str, night_action_frame: tk.Frame):
    ...
    match info_type:
        case "None":
            specific_info_box = None
        case "Number":
            specific_info_box = tk.Entry(night_action_frame, validate="key", vcmd=int_vcmd)
            specific_info_box.grid(row=3, column=1)
        case "One Character":
            specific_info_box = ttk.Combobox(night_action_frame, state="readonly", values=[c.name for c in character_list])
            specific_info_box.grid(row=3, column=1)
        case "Three Characters":
            specific_info_box = tk.Listbox(night_action_frame, selectmode="multiple", listvariable=tk.StringVar(value=[c.name for c in character_list]))
            specific_info_box.grid(row=3, column=1)

def finish_night_action(self_player: Player, chosen_player_name: str, info_type, night_action_panel: tk.Toplevel):
    ...
    if night_num == 1:
        ...
        if solver.solve(model) in (cp_model.FEASIBLE, cp_model.OPTIMAL):
            previous_night_decisions.append((self_player, number_learned, chosen_player))
        else:
    ...
    match specific_info_box:
        ...
        case tk.Entry() | ttk.Combobox():
            ...
            if final_info == '':
                final_info = False
        case tk.Listbox():
            final_info = [specific_info_box.get(i) for i in specific_info_box.curselection()]
            if len(final_info) != 3 or len(player_list) < 7:
                final_info = False
    return final_info

def first_night_model(player_making_choice: Player=None, chosen_player: Player=None):
    ...
    evil_alignment_token_indexes = [i for i, t in enumerate(token_list) if t.name in ("goon_evil",)]
    extra_evils = model.new_int_var(0, len(evil_alignment_token_indexes), "extra_evils")    
    model.add(extra_evils == sum(tokens[p][t] for t in evil_alignment_token_indexes for p in range(len(player_list))))        
    ...
    for p in range(len(player_list)):
        ...
        model.add_max_equality(p_turned_evil, [tokens[p][t] for t in evil_alignment_token_indexes])
        p_is_evil = model.new_bool_var(f"{p}_is_evil")
        model.add_bool_or(p_turned_evil, sum(assigned_char[p][j] for j, c in enumerate(character_list) if c.alignment == "evil") == 1).only_enforce_if(p_is_evil)
        model.add_bool_and(p_turned_evil.Not(), sum(assigned_char[p][j] for j, c in enumerate(character_list) if c.alignment == "evil") == 0).only_enforce_if(p_is_evil.Not())
    ...
    # must have one demon
    model.add(sum(assigned_char[p][j] for j, c in enumerate(character_list) if c.character_type=="demon" for p in range(player_count)) == 1)
    ...
    model.add(sum(assigned_char[p][balloonist_index] for p in range(player_count)) == 1).only_enforce_if(balloonist_exists)
    model.add(sum(assigned_char[p][balloonist_index] for p in range(player_count)) == 0).only_enforce_if(balloonist_exists.Not())
    model.add_allowed_assignments(
        [balloonist_exists, sum(assigned_char[p][j] for p in range(player_count) for j, c in enumerate(character_list) if c.character_type=="outsider")],
        [
            (0, outsider_count),
            (1, outsider_count),
            (1, outsider_count+1)
    ...
    for p in range(player_count):
        ...
        model.add(p_can_target == sum(assigned_char[p][j] for j, c in enumerate(character_list) if c.can_target_night_1 == True))
        p_learns_player = model.new_bool_var("")
        model.add(p_learns_player == sum(assigned_char[p][j] for j, c in enumerate(character_list) if c.learns_player == True))
        model.add(sum(target[p]) == p_can_target)
        model.add(sum(player_learned[p]) == p_learns_player)
    # tokens
    for token in token_list:
        if token.conditions is not None:
            token.conditions[0](model, player_list, token_list, tokens, character_list=character_list, assigned_char=assigned_char, target=target, current_tokens=current_tokens, is_evil=is_evil)
    
    # at most one alchemist
    alchemist_indexes = [j for j, c in enumerate(character_list) if "alchemist" in c.name]
    model.add(sum(assigned_char[p][c] for p in range(len(player_list)) for c in alchemist_indexes) <= 1)
    ...
    if player_making_choice != None:
        ...
        if chosen_player == None:
            ...
            model.add(target[player_making_choice_index][player_list.index(chosen_player)] == 1)

        for player, _, chosen_player in previous_night_decisions: # TODO: move out of if statement?
...
seat_menu.add_command(label="Create Night Action", command=lambda: create_night_action(seat_menu.current_seat))
seat_menu.add_command(label="View Possible Characters", command=lambda: print(f"Possible characters:\n{seat_menu.current_seat.player.possible_characters}"))
...
for i in range(player_count):
    ...
    seat_frame.place(x=CIRCLE_CENTRE+x_point, y=CIRCLE_CENTRE-y_point, anchor="center")
    seat = Seat(seat_frame, width=5, height=3)
    seat.bind("<Button-1>", create_seat_menu)
    seat.grid(row=0)
    seat_name = tk.Entry(seat_frame, justify="center", bg="light grey", width=15, disabledforeground="black", disabledbackground="#E0E0E0")
...
def start_night():
    ...
    if night_num == 1:
        ...
        executee_selector.config(values=[None]+[player.name for player in player_list])
...
def end_night():
    ...
    if night_num == 1:
        ...
        for i, p in enumerate(player_list):
            ...
            p.possible_characters = [character_list[c] for c in range(len(character_list)) if possible_char_indexes[i][c]]
...
start_night_button = tk.Button(night_control_frame, text="Start Night", command=start_night)
start_night_button.grid(row=0)
end_night_button = tk.Button(night_control_frame, text="End Night", command=end_night, state="disabled")
...
def toggle_alignments(alignments_enabled="change"):
    if alignments_enabled == "change":
        if seat_list[0].cget("highlightbackground") not in [EVIL_COLOUR, GOOD_COLOUR, QUANTUM_COLOUR]:
...
toggle_alignments_button = tk.Button(root, text="Toggle Alignments", command=toggle_alignments, state="disabled")
